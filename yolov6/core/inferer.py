...
class Inferer:
    def __init__(self, source, weights, device, yaml, img_size, half):
        ...
        if img_paths[0].endswith("mp4"):
            self.img_paths = img_paths[0]
        else:
    ...
    def infer_video(self, conf_thres, iou_thres, classes, agnostic_nms, max_det, save_dir, save_txt, save_img, hide_labels, hide_conf):
        ''' Model Inference and results visualization '''

        print("--->", self.img_paths)
        cap= cv2.VideoCapture(self.img_paths)
        ...
        # for img_path in tqdm(self.img_paths):
            ...
            counter += 1
            pbar.update(counter)
            img, img_src = self.precess_image_vid(frame, self.img_size, self.stride, self.half)
            img = img.to(self.device)
            if len(img.shape) == 3:
                img = img[None]
                # expand for batch dim
            ...
            save_path = osp.join(save_dir, osp.basename(self.img_paths))  # im.jpg
            txt_path = osp.join(save_dir, 'labels', osp.basename(self.img_paths).split('.')[0])
            ...
            if len(det):
                ...
                if save_img:
                    # cv2.imwrite(save_path, img_src)
                    result.write(img_src)
                    cv2.imshow("frame", img_src)
            else:
                result.write(img_ori)
    ...
    @staticmethod
    def precess_image_vid(path, img_size, stride, half):
...
class Inferer_web:
    def __init__(self, source, weights, device, yaml, img_size, half, img_data):
        import glob
        from yolov6.data.datasets import IMG_FORMATS
    ...
    def infer(self, conf_thres, iou_thres, classes, agnostic_nms, max_det, save_dir, save_txt, save_img, hide_labels, hide_conf):
        ...
        for img_path in tqdm(self.img_paths):
            img, img_src = self.precess_image(img_path, self.img_size, self.stride, self.half, self.img_data)
            img = img.to(self.device)
            if len(img.shape) == 3:
                img = img[None]
                # expand for batch dim
    ...
    @staticmethod
    def precess_image(path, img_size, stride, half, img_data_all):
        '''Process image before image inference.'''
        try:
            img_src = np.array(img_data_all) 
